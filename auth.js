...
/**
 * Update the header button / sidebar and fire authStateChanged.
...
async function _authSyncUI(user) {
  ...
  if (user) {
    ...
    // Task 3: reset role on sign-out so stale admin access is never kept.
    window.CURRENT_USER_ROLE = 'user';
...
window.handleSignIn = async function () {
  ...
  try {
    ...
    if (res.error) throw res.error;
    // _authSyncUI is now async — await it so role is set before the modal closes.
...
window.handleSignOut = async function () {
  ...
  // Passing null triggers the role reset inside _authSyncUI (Task 3).
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  // _authSyncUI handles role fetching internally before rendering UI or
  // firing authStateChanged, so all downstream code sees the correct role.
  try {
